...
struct JustStartView: View {
    ...
    var mapView: some View {
        Map(coordinateRegion: $manager.region, showsUserLocation: true).disabled(true)
            .overlay(
                Circle()
                    .frame(width: UIScreen.main.bounds.width * 2.307, height: UIScreen.main.bounds.height * 1.184, alignment: .center)
                    .foregroundColor(Color.white)
                    .mask(RadialGradient(gradient: forMapRadialGradient, center: .center, startRadius: 100, endRadius: 600))
            )
    }
